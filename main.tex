...
\end{lstlisting}
    ...
    \begin{frame}[fragile]{Использование Beamer}
   		Для использования данного шаблона необходимо подключить стилевой файл \texttt{spbu.sty}, который идет в комплекте:
...
\end{lstlisting}
        \end{block}
        Затем, после команды \verb|\begin{document}|, необходимо написать команду \verb|\maketitle|, как было показано на предыдущем слайде.
    \end{frame}
...
\end{lstlisting}
    ...
    \begin{frame}[fragile]{Добавление изображений}
    	...
    	\begin{columns}
    		\begin{column}{0.7\textwidth}
    			Используем окружение \texttt{figure}, после этого можно добавить подпись для изображения, используя команду \texttt{\textbackslash caption}, и метку, используя команду \texttt{\textbackslash label}. 
    			\begin{block}{Добавление изображения}
\begin{lstlisting}[language=TeX]
\begin{figure}[h!]
	\centering
...
\end{lstlisting}
    		...
    		\end{column}
    	\end{columns}
		Теперь можно сослаться на рис.~\ref{fig:logo}, используя команду \texttt{\textbackslash ref\{fig:logo\}}.
    \end{frame}
